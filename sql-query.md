...
2. Below the SELECT statement, the **Target** field displays where the result will be stored. An initial bucket is automatically created for you when you open the UI. It is then chosen as your default location, if your query does not specify an `INTO` clause. To ensure the automatic setup of an initial bucket, ensure the following:
...
In the *Details* tab, you can set any location that you specified in the `INTO` clause as your default location.
