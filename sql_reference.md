...
copyright:
  years: 2018
lastupdated: "2018-11-29 16:03:00"
...
<img style="max-width: 629px;" usemap="#queryImgMap" alt="railroad diagram for a query" src="./diagrams/query-1543605978315.svg" />
</div>
...
<img style="max-width: 453px;" usemap="#namedQueryImgMap" alt="railroad diagram for a named query" src="./diagrams/namedQuery-1543605978315.svg" />
</div>
...
<div style="overflow-x : auto;">
<map name="resultClauseImgMap">
	<area alt="section COSURI" shape="rect" coords="122,20,190,42" href="#COSURI" />
</map>
<img style="max-width: 501px;" usemap="#resultClauseImgMap" alt="railroad diagram for a result clause" src="./diagrams/resultClause-1543605978315.svg" />
</div>
...
<div style="overflow-x : auto;">
<map name="resultPartitionedClauseImgMap">
	<area alt="section resultPartitionedColumnClause" shape="rect" coords="188,20,440,42" href="#resultPartitionedColumnClause" />
...
<img style="max-width: 501px;" usemap="#resultPartitionedClauseImgMap" alt="railroad diagram for a result partitioned clause" src="./diagrams/resultPartitionedClause-1543605978315.svg" />
</div>

<h3 id="resultPartitionedColumnClause">resultPartitionedColumnClause</h3>

This clause causes the creation of Hive-style partitioned result data.

<div style="overflow-x : auto;">
<map name="resultPartitionedColumnClauseImgMap">
	<area alt="section identifier" shape="rect" coords="174,30,274,52" href="#identifier" />
...
<img style="max-width: 437px;" usemap="#resultPartitionedColumnClauseImgMap" alt="railroad diagram for a result partitioned column clause" src="./diagrams/resultPartitionedColumnClause-1543605978315.svg" />
</div>

<h3 id="resultPartitionedBucketClause">resultPartitionedBucketClause</h3>

<div style="overflow-x : auto;">
<map name="resultPartitionedBucketClauseImgMap">
...
<img style="max-width: 613px;" usemap="#resultPartitionedBucketClauseImgMap" alt="railroad diagram for a result partitioned bucket clause" src="./diagrams/resultPartitionedBucketClause-1543605978315.svg" />
</div>

<h3 id="resultPartitionedIntoBuckets">resultPartitionedIntoBuckets</h3>

This clause causes the creation of a fixed number of partitions in the result data.
...
<img style="max-width: 393px;" usemap="#resultPartitionedIntoBucketsImgMap" alt="railroad diagram for a result partitioned into bucket clause" src="./diagrams/resultPartitionedIntoBuckets-1543605978315.svg" />
</div>

<h3 id="resultPartitionedEveryRows">resultPartitionedEveryRows</h3>

This clause causes the creation of result partitions with a fixed number of rows per partition.
...
<img style="max-width: 377px;" usemap="#resultPartitionedEveryRowsImgMap" alt="railroad diagram for a result partitioned every rows clause" src="./diagrams/resultPartitionedEveryRows-1543605978315.svg" />
</div>

The *query result clause* allows to explicitly specify the storage location and type of a query result.
...
Using the `PARTITIONED BY (column-list)` clause without specifying `INTO x BUCKETS` allows to store the query result 
...
1. Hive-style partitions have a very eye-catching naming scheme since the column names used for partitioning are part of the partition object prefix, for example, `/order/COUNTRY=USA/part-m-00000.snappy.parquet`
...
than the partition representing order data from the USA. Thus, if your primary concern regarding physical layout is *equal sized* partitions, 
you should bucketize the query result.   


<h4>Partition by columns into buckets</h4>

Bucketizing a query result allows to specify the exact number of *equally-sized* result partitions. This allows to experimentally fine tune the number of buckets to meet certain criteria for partition size. To specify the number of partitions use the `PARTITIONED INTO x BUCKETS` clause. 
...
<img style="max-width: 502px;" usemap="#fullselectImgMap" alt="railroad diagram for a fullselect" src="./diagrams/fullselect-1543605978315.svg" />
</div>
...
* `SORT BY`: The *sort by* clause allows to define an overall ordering on a partition base as defined by the list of `expression` clauses. The default order direction is ascending if not explicitly specified by an `expression` clause.  The *sort by* clause is used in conjunction with the *distribute by* clause.
...
<img style="max-width: 594px;" usemap="#simpleselectImgMap" alt="railroad diagram for a simpleselect" src="./diagrams/simpleselect-1543605978315.svg" />
</div>

The simpleselect allows to specify the following characteristics of a result set:
...
<img style="max-width: 437px;" usemap="#resultColumnImgMap" alt="railroad diagram for a result column" src="./diagrams/resultColumn-1543605978315.svg" />
</div>
...
<img style="max-width: 591px;" usemap="#groupByClauseImgMap" alt="railroad diagram for a group by clause" src="./diagrams/groupByClause-1543605978315.svg" />
</div>
...
<img style="max-width: 417px;" usemap="#groupingSetImgMap" alt="railroad diagram for a grouping set" src="./diagrams/groupingSet-1543605978315.svg" />
</div>
...
A GROUPING SETS grouping is an extension to the *group by clause* that allows to explicitly specify the grouping sets of interest. In other words, the ROLLUP and CUBE groupings are shortcuts for common grouping set use cases.

Referring to the above example, adding a `GROUPING SETS` modifier to the *group by* clause allows to compute *rollup* sales data on a quarter by year basis and a yearly basis only as shown below.
...
<img style="max-width: 497px;" usemap="#relationImgMap" alt="railroad diagram for a relation" src="./diagrams/relation-1543605978315.svg" />
</div>
...
<img style="max-width: 487px;" usemap="#joinClauseImgMap" alt="railroad diagram for a join clause" src="./diagrams/joinClause-1543605978315.svg" />
</div>
...
<img style="max-width: 502px;" usemap="#naturalJoinClauseImgMap" alt="railroad diagram for a natural join clause" src="./diagrams/naturalJoinClause-1543605978315.svg" />
</div>
...
<map name="relationPrimaryImgMap">
	<area alt="section externalTableSpec" shape="rect" coords="146,40,302,62" href="#externalTableSpec" />
	<area alt="section sample" shape="rect" coords="332,40,400,62" href="#sample" />
	<area alt="section identifier" shape="rect" coords="526,40,626,62" href="#identifier" />
	<area alt="section identifier" shape="rect" coords="90,90,190,112" href="#identifier" />
	<area alt="section identifier" shape="rect" coords="278,90,378,112" href="#identifier" />
	<area alt="section sample" shape="rect" coords="408,90,476,112" href="#sample" />
	<area alt="section identifier" shape="rect" coords="602,90,702,112" href="#identifier" />
	<area alt="section fullselect" shape="rect" coords="174,140,274,162" href="#fullselect" />
	<area alt="section sample" shape="rect" coords="352,140,420,162" href="#sample" />
	<area alt="section identifier" shape="rect" coords="546,140,646,162" href="#identifier" />
	<area alt="section relation" shape="rect" coords="182,190,266,212" href="#relation" />
	<area alt="section sample" shape="rect" coords="344,190,412,212" href="#sample" />
	<area alt="section identifier" shape="rect" coords="538,190,638,212" href="#identifier" />
...
<img style="max-width: 793px;" usemap="#relationPrimaryImgMap" alt="railroad diagram for a relation primary" src="./diagrams/relationPrimary-1543605978315.svg" />
</div>
...
<img style="max-width: 469px;" usemap="#externalTableSpecImgMap" alt="railroad diagram for an external table specification" src="./diagrams/externalTableSpec-1543605978315.svg" />
</div>

<h3 id="tableValuedFunction">tableValuedFunction</h3>

A table-valued function returns a relation, that is, a set of rows. An example of a table-valued function is `range()`. Refer to the section about [SQL functions](../sqlfunctions.html#sql-functions) for more details.
...
<img style="max-width: 497px;" usemap="#tableValuedFunctionImgMap" alt="railroad diagram for a table valued function" src="./diagrams/tableValuedFunction-1543605978315.svg" />
</div>
...
<img style="max-width: 565px;" usemap="#valuesClauseImgMap" alt="railroad diagram for a values clause" src="./diagrams/valuesClause-1543605978315.svg" />
</div>

A values clause allows to define a result set by specifying actual values for each column of a row by means of expressions.
...
<img style="max-width: 537px;" usemap="#lateralViewImgMap" alt="railroad diagram for a lateral view" src="./diagrams/lateralView-1543605978315.svg" />
</div>
...
<img style="max-width: 301px;" usemap="#joinTypeImgMap" alt="railroad diagram for join types" src="./diagrams/joinType-1543605978315.svg" />
</div>
...
<img style="max-width: 553px;" usemap="#sortItemImgMap" alt="railroad diagram for a sort item" src="./diagrams/sortItem-1543605978315.svg" />
</div>
...
<img style="max-width: 957px;" usemap="#functionOrAggregateImgMap" alt="railroad diagram for a function or aggregate" src="./diagrams/functionOrAggregate-1543605978315.svg" />
</div>
...
Refer to section [SQL functions](../sqlfunctions.html#sql-functions) for details about supported functions.
...
Refer to the section about [SQL functions](../sqlfunctions.html#sql-functions) for more detailed information.

A window can be defined in two ways:
* The `WINDOW` keyword allows to define an identifier for a window specification in a *fullselect* or *simpleselect*. This named window specification can then be referenced by the `OVER` keyword.
...
<img style="max-width: 337px;" usemap="#namedWindowsImgMap" alt="railroad diagram for named windows" src="./diagrams/namedWindows-1543605978315.svg" />
</div>
...
<img style="max-width: 377px;" usemap="#namedWindowImgMap" alt="railroad diagram for a named window" src="./diagrams/namedWindow-1543605978315.svg" />
</div>
...
<img style="max-width: 765px;" usemap="#windowSpecImgMap" alt="railroad diagram for a window specification" src="./diagrams/windowSpec-1543605978315.svg" />
</div>
...
<img style="max-width: 393px;" usemap="#windowClusterByImgMap" alt="railroad diagram for a window cluster by clause" src="./diagrams/windowClusterBy-1543605978315.svg" />
</div>
...
<img style="max-width: 437px;" usemap="#windowPartitionByImgMap" alt="railroad diagram for a window partition by clause" src="./diagrams/windowPartitionBy-1543605978315.svg" />
</div>
...
<img style="max-width: 381px;" usemap="#windowOrderByImgMap" alt="railroad diagram for a window order by clause" src="./diagrams/windowOrderBy-1543605978315.svg" />
</div>
...
<img style="max-width: 601px;" usemap="#windowFrameImgMap" alt="railroad diagram for a window frame" src="./diagrams/windowFrame-1543605978315.svg" />
</div>
...
<img style="max-width: 373px;" usemap="#frameBoundImgMap" alt="railroad diagram for a frame boundary" src="./diagrams/frameBound-1543605978315.svg" />
</div>
...
Note that the keyword `OVER` allows to define an unnamed window specification in a [functionOrAggregate](#functionOrAggregate)
...
<img style="max-width: 257px;" usemap="#expressionImgMap" alt="railroad diagram for an expression" src="./diagrams/expression-1543605978315.svg" />
</div>
...
<img style="max-width: 537px;" usemap="#booleanExpressionImgMap" alt="railroad diagram for a boolean expression" src="./diagrams/booleanExpression-1543605978315.svg" />
</div>
...
<img style="max-width: 625px;" usemap="#valueExpressionImgMap" alt="railroad diagram for a value expression" src="./diagrams/valueExpression-1543605978315.svg" />
</div>
...
<img style="max-width: 553px;" usemap="#primaryExpressionImgMap" alt="railroad diagram for a primary expression" src="./diagrams/primaryExpression-1543605978315.svg" />
</div>
...
<img style="max-width: 329px;" usemap="#constantImgMap" alt="railroad diagram for a constant" src="./diagrams/constant-1543605978315.svg" />
</div>

<h4 id="interval">interval</h4>

An *interval clause* allows to define time duration constants that can be used in expressions to add or substract time ranges from a timestamp value.

<div style="overflow-x : auto;">
<map name="intervalImgMap">
	<area alt="section INTEGER&#95;VALUE" shape="rect" coords="292,30,416,52" href="#INTEGER&#95;VALUE" />
	<area alt="section DECIMAL&#95;VALUE" shape="rect" coords="292,60,416,82" href="#DECIMAL&#95;VALUE" />
...
<img style="max-width: 633px;" usemap="#intervalImgMap" alt="railroad diagram for an interval" src="./diagrams/interval-1543605978315.svg" />
</div>
...
<img style="max-width: 237px;" usemap="#timeUnitSpecImgMap" alt="railroad diagram for a time unit specification" src="./diagrams/timeUnitSpec-1543605978315.svg" />
</div>
...
<img style="max-width: 361px;" usemap="#columnReferenceImgMap" alt="railroad diagram for a column reference" src="./diagrams/columnReference-1543605978315.svg" />
</div>
...
<img style="max-width: 241px;" usemap="#qualifiedNameImgMap" alt="railroad diagram for a qualified name" src="./diagrams/qualifiedName-1543605978315.svg" />
</div>
...
<img style="max-width: 685px;" usemap="#predicateImgMap" alt="railroad diagram for a predicate" src="./diagrams/predicate-1543605978315.svg" />
</div>
...
<img style="max-width: 529px;" usemap="#castExpressionImgMap" alt="railroad diagram for a cast expression" src="./diagrams/castExpression-1543605978315.svg" />
</div>
...
<map name="dataTypeImgMap">
	<area alt="section dataType" shape="rect" coords="278,20,362,42" href="#dataType" />
	<area alt="section dataType" shape="rect" coords="194,50,278,72" href="#dataType" />
	<area alt="section dataType" shape="rect" coords="346,50,430,72" href="#dataType" />
	<area alt="section complexColTypeList" shape="rect" coords="242,90,406,112" href="#complexColTypeList" />
...
<img style="max-width: 561px;" usemap="#dataTypeImgMap" alt="railroad diagram for a data type" src="./diagrams/dataType-1543605978315.svg" />
</div>
...
<img style="max-width: 273px;" usemap="#complexColTypeListImgMap" alt="railroad diagram for a complex column type list" src="./diagrams/complexColTypeList-1543605978315.svg" />
</div>
...
<img style="max-width: 577px;" usemap="#complexColTypeImgMap" alt="railroad diagram for a complex column type" src="./diagrams/complexColType-1543605978315.svg" />
</div>
...
<img style="max-width: 749px;" usemap="#caseExpressionImgMap" alt="railroad diagram for a case expression" src="./diagrams/caseExpression-1543605978315.svg" />
</div>
...
<img style="max-width: 465px;" usemap="#whenClauseImgMap" alt="railroad diagram for a when clause" src="./diagrams/whenClause-1543605978315.svg" />
</div>
...
| `~A`      | All number types | Returns bitwise NOT A. The type of the result is the same as the type of A. |
...
| Operator | Operand types | Description |
| :----: | ---- | ---- |
| `A \|\| B`|  String types | Returns the concatenation of strings A and B. If A or B is not a string, it is first converted into a string type. The result is a string. |
...
Any table (that is, any object that is stored on Cloud {{site.data.keyword.cos_short}}) that is used in a *from* clause can be associated with a *table sample* clause.
...
This lets you write queries for samples of the data, for example, for the interactive data exploration, data mining, and so on.
...
<map name="sampleImgMap">
	<area alt="section INTEGER&#95;VALUE" shape="rect" coords="256,20,380,42" href="#INTEGER&#95;VALUE" />
	<area alt="section DECIMAL&#95;VALUE" shape="rect" coords="256,50,380,72" href="#DECIMAL&#95;VALUE" />
...
<img style="max-width: 605px;" usemap="#sampleImgMap" alt="railroad diagram for a sample" src="./diagrams/sample-1543605978315.svg" />
</div>
...
<img style="max-width: 566px;" usemap="#bucketSampleClauseImgMap" alt="railroad diagram for a bucket sample clause" src="./diagrams/bucketSampleClause-1543605978315.svg" />
</div>

Three sampling types are supported:

* `TABLESAMPLE <number> PERCENT` allows to sample a certain percentage of rows from the underlying table.
* `TABLESAMPLE <expression> ROWS` allows to sample a certain number of rows from the underlying table.
* `TABLESAMPLE BUCKET x OUT OF y` allows to bucketize the underlying data into `y` buckets and returns rows from bucket `x`. Buckets are numbered from `1`to `y`.
...
The syntax of a COS URI is thoroughly described in section [Table unique resource identifier](../sql-query.html#table-unique-resource-identifier).
